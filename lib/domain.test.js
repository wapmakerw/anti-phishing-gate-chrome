...
describe("SandboxDomain library", () => {
  ...
  describe("confusableRisk", () => {
    ...
    test("flags IP-address literals used in place of a domain name", () => {
      expect(SandboxDomain.confusableRisk("192.168.0.1")).not.toBeNull();
    });

    test("does not flag benign infrastructure labels", () => {
      expect(SandboxDomain.confusableRisk("www.example.com")).toBeNull();
      expect(SandboxDomain.confusableRisk("api.example.com")).toBeNull();
      expect(SandboxDomain.confusableRisk("cdn.example.com")).toBeNull();
    });
